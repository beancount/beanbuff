...
def fetch_page_data(fund: str, year: int):
    ...
    url = urllib.parse.urlunparse(
        (
            ...
            "/us/funds/tools/pricehistorysearch",
            "",
    ...
    if _CACHE:
        ...
        if path.exists(filename):
            ...
            output = subprocess.check_output(
                command, encoding="utf8", stderr=subprocess.DEVNULL
            )
...
def fetch_data(
    fund: str, requested_year: Optional[int]
) -> List[Tuple[datetime.date, Decimal]]:
    """Fetch all data for a specific fund."""
    year_prices = []
    for index, year in enumerate(
        range(datetime.date.today().year, 0, -1)
...
def main():
    ...
    parser.add_argument("funds", nargs="+", help="Four-digit fund numbers.")
    parser.add_argument(
        "-o", "--output-dir", action="store", help="Output directory name."
    )
    parser.add_argument(
        "-r",
        "--restricted-dates",
    ...
    parser.add_argument(
        "-y", "--year", action="store", type=int, help="Fetch for a single, given year"
    )
        ...
        if args.output_dir:
            ...
            for date, price in prices:
                if args.restricted_dates and date not in dates:
                    continue
                print(
                    "{:%Y-%m-%d} price {} {} {}".format(date, instrument, price, "USD")
                )
            print()
