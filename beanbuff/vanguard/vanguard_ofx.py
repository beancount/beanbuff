...
class Importer(identifier.IdentifyMixin, filing.FilingMixin, config.ConfigMixin):
    ...
    matchers = [
        ('mime', 'application/x-ofx|application/vnd.intu.qfx'),
        ('content', 'Vanguard'),
        ('content', '<ACCTID>'),
    ]
            ...
            for currency_node in stmtrs.find_all('curdef'):
                ...
                for invtranlist in stmtrs.find_all(re.compile('(invtranlist)')):
                    ...
                    for tran in invtranlist.find_all(re.compile('(buymf|sellmf|reinvest|buystock|sellstock|buyopt|sellopt|transfer)')):
                        ...
                        entry = data.Transaction(fileloc, date, self.FLAG, payee, narration, EMPTY_SET, EMPTY_SET, [])
                    ...
                    for tran in invtranlist.find_all(re.compile('(invbanktran)')):
                        ...
                        entry = data.Transaction(fileloc, date, self.FLAG, None, narration, EMPTY_SET, {fitid}, [
...
if __name__ == '__main__':
    importer = Importer(filing='Assets:US:Vanguard:Retire:PreTax',
                        matchers=[('content', '<ACCTID>87654321')],
                        config={
        'account_id'          : '87654321',
